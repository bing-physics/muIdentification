...
public:
  extrapolator(double bfield){B=bfield;}
...
void extrapolator::setRZYC_charge_pos_b(double R, double ZC, double YC, double charge, const TVector3 &pos, double Px2pyz){
  r=R/10.;   
  zc=ZC/10.;
  yc=YC/10.;
  Cha=charge;
...
void extrapolator::setPosMomCha(const TVector3 &pos, const TVector3 &mom,  double charge){
...
void extrapolator::Go(double lenyz, TVector3 &pos){
...
void extrapolator::plot(const char* name){
...
double extrapolator::gettrklength(){
  return dPhi*TMath::Pi()/180.*r*sqrt(1+px2pyz*px2pyz);
}

double extrapolator::extrapolate2outermost(TVector3 &pos){
  double centerY=-238.473;  // cm
  double centerZ=2391; // cm
...
bool  extrapolator::extrapolate2ECALlayer(int ecallayer, double &trklength, TVector3 &pos){
  double sttr=200;
  double thickness[5]={4.4,4.4,4.4,4.4,5.28};
  double r6[6];
  r6[0]=sttr;
...
bool  extrapolator::extrapolate2ECALlayer(double rr, double &trklength , TVector3 &pos){
  /*
  double sttr=200;
  double thickness[5]={4.4,4.4,4.4,4.4,5.28};
  double r6[6];
